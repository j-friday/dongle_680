...
void prf_gatt_write(prf_env_t *prf_env, uint8_t conidx,
                    uint16_t handle, uint8_t* value, uint16_t length, uint8_t operation,uint8_t seq_num)
{
...
void prf_gatt_write_ntf_ind(prf_env_t *prf_env, uint8_t conidx, uint16_t handle, uint16_t ntf_ind_cfg,uint8_t seq_num)
{
    uint8_t value[2];
    ...
    prf_gatt_write(prf_env, conidx, handle, value, 2, GATTC_WRITE,seq_num);
}
