...
uint8_t send_data_enable_flag[BLE_CONNECTION_MAX] = {0};/// = 1;
        ...
        ke_task_id_t const src_id)
{
    ...
    bk_printf("[elect] gattc_att_info_req_ind_handler:att_idx:0x%x\r\n",att_idx);
                                      ...
                                      ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    bk_printf("[electric] %s msgid:0x%x,dest_id:0x%x,src_id:0x%x,conidx:%x\r\n",__func__,msgid,dest_id,src_id,conidx);
    // retrieve handle information
    uint8_t status = electric_get_att_idx(param->handle, &att_idx);
		
    bk_printf("param->handle:%d,att_idx:%d,status:%d\r\n",param->handle,att_idx,status);
                                      ...
                                      ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    bk_printf("[electric]%s,dest:0x%x,src_id:0x%x,conidx:%d,handle:%d\r\n",__func__,dest_id,src_id,conidx,param->handle);
...
void electric_task_init(struct ke_task_desc *task_desc)
{
    // Get the address of the environment
    struct electric_env_tag *srv_env = PRF_ENV_GET(ELECTRIC, electric);

    task_desc->msg_handler_tab = electric_msg_handler_tab;
    task_desc->msg_cnt         = ARRAY_LEN(electric_msg_handler_tab);
    task_desc->state           = srv_env->state;
    task_desc->idx_max         = ELECTRIC_IDX_MAX;
}
