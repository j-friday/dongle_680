...
struct add_profiles_t
{
    uint16_t add_profiles_nums[BLE_NB_PROFILES_ADD_MAX];
...
{
    uint16_t add_profiles_nums[BLE_ACTIVITY_MAX];
...
struct prf_sdp_db_env* sdp_extract_svc_info(uint8_t conidx,struct gattc_sdp_svc_ind const *param);
