                                            ...
                                            ke_task_id_t const src_id)
{
    ...
    if(state == FCC0S_IDLE)
    {
        ...
        if(fcc0s_env->ntf_cfg[conidx] == PRF_CLI_START_NTF)
        {
            ...
            fcc0s_notify_fcc2_val(conidx,fcc0s_env, param);
    ...
    if (status == GAP_ERR_NO_ERROR)
    {
                ...
                && ((ntf_cfg == PRF_CLI_STOP_NTFIND) || (ntf_cfg == PRF_CLI_START_NTF)))
        {
            ...
            if (ntf_cfg == PRF_CLI_START_NTF)
            {
                ...
                fcc0s_env->ntf_cfg[conidx] = PRF_CLI_START_NTF ;
            }
            else
            {
                                      ...
                                      ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    uint16_t length = 0;
    struct fcc0s_env_tag* fcc0s_env = PRF_ENV_GET(FCC0S, fcc0s);
    bk_printf("[fcc0]%s conidx:%d,handle:%d,att_idx:%d\r\n",__func__,conidx,param->handle,att_idx);
    // If the attribute has been found, status is GAP_ERR_NO_ERROR
    if (status == GAP_ERR_NO_ERROR)
    {
        ...
        // read notification information
        else if (att_idx == FCC0S_IDX_FCC2_VAL_NTF_CFG)
        {
