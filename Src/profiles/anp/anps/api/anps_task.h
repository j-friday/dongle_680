...
enum anps_msg_ids
{
    ...
    ANPS_ENABLE_REQ = TASK_FIRST_MSG(TASK_ID_ANPS),//!< ANPS_ENABLE_REQ
    /// Start the Alert Notification Profile Server Role
    ANPS_ENABLE_RSP,                               //!< ANPS_ENABLE_RSP

    /// Update the value of a characteristic
    ANPS_NTF_ALERT_CMD,                            //!< ANPS_NTF_ALERT_CMD
    /// The peer device requests to be notified about new alert values
    ANPS_NTF_IMMEDIATE_REQ_IND,                    //!< ANPS_NTF_IMMEDIATE_REQ_IND
    /// Indicate that the notification configuration has been modified by the peer device
    ANPS_NTF_STATUS_UPDATE_IND,                    //!< ANPS_NTF_STATUS_UPDATE_IND

    /// Complete Event Information
    ANPS_CMP_EVT,                                  //!< ANPS_CMP_EVT
};
