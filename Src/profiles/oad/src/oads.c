...
//	uint8_t testbuffer[16];
	...
	{	
	  ...
	  flash_write(FLASH_MAIN_BASE_ADDR, wr_addr, len, sec_ptr->data, NULL);
		...
		bk_printf("ImgHdr.rom_ver = %x \r\n", ImgHdr.rom_ver);
		bk_printf("ImgHdr.uid = %x \r\n", ImgHdr.uid);
		bk_printf("ImgHdr.ver = %x \r\n", ImgHdr.ver);
		bk_printf("ImgHdr.LEN = %x \r\n", ImgHdr.len);
		for(int i = 0 ;i < 10;i++)
		{
			bk_printf("wait for reset!!!\r\n");
		}	
		bsec.flag_write = 0;
		wdt_enable(10);
		while(1);
...
uint8_t oadImgIdentifyWrite( uint16_t connHandle, uint16_t length,uint8_t *pValue )
{
	...
	bk_printf("rxHdr.ver = %x \r\n", rxHdr.ver);
	bk_printf("rxHdr.len = %x \r\n", rxHdr.len);
	bk_printf("rxHdr.uid = %x \r\n", rxHdr.uid);
	bk_printf("rxHdr.rom_ver = %x \r\n", rxHdr.rom_ver);			
	
	if(rxHdr.uid == OAD_APP_PART_UID)
	{
		oad_firmware_type = 1;
		bk_printf("app part upgrade\r\n");
	...
	{
		oad_firmware_type = 2;
		bk_printf("app and stack upgrade\r\n");
	...
	else
	{
		oad_firmware_type = 0;
		flash_read(0,SEC_IMAGE_OAD_HEADER_APP_FADDR, sizeof(img_hdr_t),(uint8_t *)&ImgHdr, NULL);
		uint8_t *ptr = (uint8_t *)&ImgHdr;
		
		bk_printf("ImgHdr = ");
		for(int i =0 ;i < sizeof(img_hdr_t);i++)
		{
			bk_printf("%02x ",ptr[i]);
		}
		bk_printf("\r\n");
...
uint8_t oadImgBlockWrite( uint16_t connHandle, uint8_t *pValue )
{
    ...
    if ( blkNum == 0 )
    {
		...
		if(oad_firmware_type == 1)
		{
			...
			bk_printf("uid = %x\r\n", uid);
			bk_printf("blkNum = %x ,oadBlkNum %x,oadBlkTot %x,ver %x \r\n",blkNum,oadBlkNum,oadBlkTot,ver);
			bk_printf("ImgHdr.ver = %x, ver = %x\r\n",ImgHdr.ver, ver);
		...
		{
			bk_printf("oadImgBlockWrite: UNKNOWN UID\r\n");
			return (0x03);
		}
...
static void oadImgIdentifyReq(uint16_t connHandle, img_hdr_t *pImgHdr)
{
	...
	if(cmd == NULL)
	{
		while(1)
		{
			bk_printf("KE_MSG_ALLOC_DYN GATTC_SEND_EVT_CMD  == NULL \r\n");
		}
