...
static int oads_ffc1_upd_req_handler(ke_msg_id_t const msgid,
                                            ...
                                            ke_task_id_t const src_id)
{
	bk_printf("oads_ffc1_upd_req_handler \r\n");
 ...
 //  bk_printf("oad %s \r\n",__func__);
   ...
   uint8_t conidx = KE_IDX_GET(src_id);
    ...
    if(ke_state_get(dest_id) == OADS_IDLE)
    {
		...
		if(oads_env == NULL)
		{
			while(1){ bk_printf("oads_env == null\r\n");};
		}
		...
		if(((oads_env->features & 0x01) == OADS_NTF_SUP) && ((ntf_cfg == PRF_CLI_STOP_NTFIND) || (ntf_cfg == PRF_CLI_START_NTF)))
		{
			...
			{	
				//bk_printf("OADS_IDX_FFC1_LVL_NTF_CFG ntf_cfg = %d\r\n",ntf_cfg);
			...
			{
			 	//bk_printf("OADS_IDX_FFC2_LVL_NTF_CFG ntf_cfg = %d\r\n",ntf_cfg);
				if (ntf_cfg == PRF_CLI_START_NTF)
				{
						// Ntf cfg bit set to 1
                                      ...
                                      ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
	...
	if(ke_state_get(dest_id) == OADS_IDLE)
	{
		...
		else if(param->handle == (oads_env->oads_start_hdl + OADS_IDX_FFC2_USER_DECL))
		{
			...
			//bk_printf("Read c2 DECL val\r\n");
		}
			
		else
		{
			  //bk_printf("Read status ATT_ERR_REQUEST_NOT_SUPPORTED \r\n");
		...
		// Send value to peer device.
	  	...
	  	//bk_printf("oad read send cfm\r\n");
		
	}
	else if(ke_state_get(dest_id) == OADS_BUSY)
	{
			bk_printf("oads OADS_BUSY \r\n");
			msg_status = KE_MSG_SAVED;
	}
		
	//bk_printf("oads %s end!\r\n",__func__);
