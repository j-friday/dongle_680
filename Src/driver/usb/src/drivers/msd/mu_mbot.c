				       ...
				       MGC_MsdCmdSet* pCmdSet)
{
    ...
    {
		os_printf("MGC_CreateBotInstance: malloc failed\r\n");
    }
...
void MGC_DestroyBotInstance(MGC_MsdProtocol* pProtocol)
{
    ...
    os_printf("MGC_DestroyBotInstance: pBot->bLunCount=%d\r\n", pBot->bLunCount);
				    ...
				    uint8_t bIsReady)
{
    ...
    os_printf("MGC_BotProtocolSetReady\r\n");
...
//    os_printf("MGC_BotControlIrpComplete start\r\n");
    if(!pBot || (pBot != pBot->pSelf)) 
		return 1;

//    os_printf("MGC_BotControlIrpComplete: pBot->bInit = 0x%x\r\n", pBot->bInit);
    if(pBot->bInit)
    {
...
//        os_printf("MGC_BotControlIrpComplete: pBot->aVolume = 0x%p\r\n", pBot->aVolume);
		if(pBot->aVolume)
		{
...
//即函数 : MGC_ScsiCmdSetCreateInstance()				
				...
				{
//                    os_printf("MGC_BotControlIrpComplete: MGC_MsdBotVolumeInit\r\n");
				    pVolume->pBot = pBot;
				    pVolume->bLun = bIndex;
...
//发送完成后,会产生中断,在BSR中则会调用 pVolume->CbwIrp.pfIrpComplete = MGC_BotCbwComplete;
		...
		{
			os_printf("MGC_BotControlIrpComplete: malloc failed\r\n");
		}
    }
