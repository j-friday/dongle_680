...
#else
    UART_PRINTF("Write16(%lx, %04x, %04x)\n", pBase, wOffset, wDatum);
#endif
...
#else
    UART_PRINTF("Write32(%lx, %04x, %08x)\n", pBase, wOffset, dwDatum);
#endif
...
{
    /* no realloc */
    void* pNewBlock = (int *)MUSB_MemAlloc(iSize);
    if (pNewBlock)
    {
    ...
    {
		os_printf("MGC_AfsMemRealloc: malloc failed\r\n");
    }
    return (pNewBlock);
}
...
void MGC_AfsUdsIsr(uint16_t wIntrTxValue,uint16_t wIntrRxValue,uint8_t bIntrUsbValue)
{
    uint8_t bIndex;
    MGC_AfsUds* pUds;
//    os_printf("MGC_AfsUdsIsr\r\n");
//	os_printf("MGC_AfsUdsIsr: MGC_bAfsUdsCount = %x\r\n",MGC_bAfsUdsCount);
			       ...
			       uint8_t** ppBaseIsr, uint8_t** ppBaseBsr)
{
   ...
   os_printf("MUSB_BoardInitController\r\n");
...
//    uHALr_RequestInterrupt (pUds->dwIrq, MGC_AfsUdsIsr, (unsigned char *)pUds->aIsrName);
 ...
 //   uHALr_EnableInterrupt(pUds->dwIrq);
    ...
    os_printf("MUSB_BoardInitController: pUds = 0x%lx, &pUds = 0x%lx\r\n", *(unsigned long *)pUds.aIsrName, (unsigned long)&pUds);
    os_printf("MUSB_BoardInitController: MGC_bAfsUdsCount = %d, MGC_apAfsUds[] = 0x%p\r\n", MGC_bAfsUdsCount, MGC_apAfsUds[MGC_bAfsUdsCount-1]);
    return &pUds;
}
...
//    UART_PRINTF("MGC_AfsTimerExpired: pUds = 0x%p\r\n", pUds);
	if (pUds == NULL)
    {
        os_printf("MGC_AfsTimerExpired: pUds = null\r\n");
		return;
    }
	pWrapper = &(pUds->aTimerWrapper[0]);
//    UART_PRINTF("MGC_AfsTimerExpired: pUds = 0x%p, pWrapper = 0x%p\r\n", pUds, pWrapper);
//	os_printf("++++ time=%x,bTimerStart=%d +++\r\n",pWrapper->dwTime,pWrapper->bTimerStart);
	if (pWrapper != NULL)
	{
        if (pWrapper->bTimerStart == 1)
        {
            ...
            {
//                UART_PRINTF("MGC_AfsTimerExpired: time up, pWrapper->pfExpired = 0x%p\r\n",
//                    pWrapper->pfExpired);
			     ...
			     void* pParam)
{
	...
	os_printf("MUSB_BoardArmTimer: wIndex=%d, dwTime=%ld, bPeriodic=%d\r\n", wIndex, dwTime, bPeriodic);
