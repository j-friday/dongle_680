...
//os_printf("mgc_hostenumerator--:%x\r\n",pEnumData->bState);
	...
	os_printf("MGC_HostEnumerator start\r\n");
    ...
    {
    	os_printf("MGC_HostEnumerator: error:irp.dwstatus = 0x%x\r\n",pEnumData->Irp.dwStatus);
	...
	{
	    os_printf("MGC_HostEnumerator: error retry\r\n");
...
//os_printf("malloc size2=== end\r\n");
	...
	{
		UART_PRINTF("MGC_HostEnumerator: malloc 1 failed\r\n");
	}
	MUSB_MemFree(pEnumData->pDevice->pPrivateData);
...
//		os_printf("pEnumdata->dwData = %x,wlengh=%x\r\n",pEnumData->dwData,wLength);
	    ...
	    {
		UART_PRINTF("MGC_HostEnumerator: malloc 2 failed\r\n");
		MGC_DIAG(1, pController, "Insufficient memory for new device");
		MUSB_MemFree(pEnumData->pDevice->pPrivateData);
		pEnumData->pDevice->pPrivateData = NULL;
		break;
...
/*运行到此处时，刚完成get configuration(full)整个过程,接下来是set configuration()*/
    ...
    os_printf("MGC_HostEnumerator end\r\n");
	return 0;//Ret;
}
...
//	os_printf("++++++ start emunate malloc size:%d ++++++++++\r\n",sizeof(MGC_Device));
    ...
    {
		UART_PRINTF("MGC_EnumerateDevice: malloc failed\r\n");
    }
    return FALSE;
}
			   ...
			   uint8_t* abInterfaceNumber, uint8_t bInterfaceCount)
{
    ...
    if(!pVirtualDevice)
    {
		UART_PRINTF("MUSB_RejectInterfaces: malloc 1 failed\r\n");
    ...
    {
		UART_PRINTF("MUSB_RejectInterfaces: malloc 2 failed\r\n");
    ...
    {
		UART_PRINTF("MUSB_RejectInterfaces: malloc 3 failed\r\n");
...
uint8_t MGC_StartNextControlTransfer(MGC_Port* pPort)
{
    ...
    os_printf("MGC_StartNextControlTransfer\r\n");
    /* NOTE: assumes EP0 is first */
    pEnd = MUSB_ArrayFetch(&(pPort->LocalEnds), 0);
    if(pEnd)
    {
					      ...
					      uint32_t dwBusTime)
{
	...
	{
		UART_PRINTF("MGC_FindScheduleSlot: malloc failed\r\n");
	}
    }
    else
    {
			...
			MUSB_EndpointResource* pEndpointResource)
{
    ...
    {
				UART_PRINTF("MUSB_OpenPipe: malloc failed\r\n");
    }
...
uint32_t MUSB_StartControlTransfer(MUSB_Port* pPort, MUSB_ControlIrp* pIrp)
{
    ...
    os_printf("MUSB_StartControlTransfer: pEnd = 0x%p, pEnd->pTxIrp = 0x%p\r\n", pEnd, pEnd->pTxIrp);
    if(pEnd)
    {
		if(!pEnd->pTxIrp)
		{
