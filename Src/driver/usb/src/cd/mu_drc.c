...
//	static MGC_EndpointResource resource[5];
    ...
    for (bEnd = 1; bEnd < MGC_MAX_USB_ENDS; bEnd++)
    {
        ...
        os_printf("MGC_DrcInit: bFifoSize = 0x%x\r\n", bFifoSize);
        if (!bFifoSize)
        {
        ...
        pPort->bEndCount++;
		os_printf("MGC_DrcInit: pPort->bEndCount = %d\r\n", pPort->bEndCount);
        pPort->wEndMask |= (1 << bEnd);
    }
//	pPort->pPrivateData = (void *)(&resource[0]);

	os_printf("MGC_DrcInit: pPort->bEndcount = 0x%x, sizeof(Endpointresource) = 0x%x\r\n", pPort->bEndCount, sizeof(MGC_EndpointResource));
    ...
    if (pPort->pPrivateData)
    {
            ...
            pPort->bEndCount, pPort->pPrivateData))
        {
            ...
            os_printf("MGC_DrcInit: MUSB_ArrayAppend1\r\n");
            os_printf("MGC_DrcInit: pPort->bEndCount = %d\r\n", pPort->bEndCount);
...
//                end.bTrafficType = end.bRxTrafficType = MUSB_ENDPOINT_XFER_ISOC;
                ...
                os_printf("MGC_DrcInit: MUSB_ArrayAppend2\r\n");
                MUSB_ArrayAppend(&(pPort->LocalEnds), &end);
            }
        }
        else
        {
            os_printf("MGC_DrcInit: MUSB_ArrayInit failed\r\n");
					  ...
					  uint8_t bBind, uint8_t* pbIsTx)
{
    ...
    os_printf("MGC_DrcBindEndpoint: dwEndCount = %ld\r\n", dwEndCount);
//    UART_PRINTF("dwEndCount=%ld\r\n", dwEndCount);
    for (dwEndIndex = 0; dwEndIndex < dwEndCount; dwEndIndex++)
    {
        pEnd = (MGC_EndpointResource*)MUSB_ArrayFetch(&(pPort->LocalEnds), dwEndIndex);
    //        os_printf("MGC_DrcBindEndpoint: pEnd = 0x%lx\r\n", (unsigned long)pEnd);
        bIsClaimed = pEnd->bIsFifoShared ? 
            pEnd->bIsClaimed : (bIsTx ? pEnd->bIsClaimed : pEnd->bRxClaimed);
    //        os_printf("MGC_DrcBindEndpoint: pEnd->bIsFifoShared = 0x%x\r\n", pEnd->bIsFifoShared);
    //        os_printf("MGC_DrcBindEndpoint: pEnd->bIsClaimed = 0x%x\r\n", pEnd->bIsClaimed);
    //        os_printf("MGC_DrcBindEndpoint: bIsTx = 0x%x\r\n", bIsTx);
    //        os_printf("MGC_DrcBindEndpoint: pEnd->bRxClaimed = 0x%x\r\n", pEnd->bRxClaimed);
    //        os_printf("MGC_DrcBindEndpoint: bIsClaimed = 0x%x\r\n", bIsClaimed);
        if (pEnd && !bIsClaimed)
        {
				 ...
				 uint16_t* pwCsr, uint16_t wCount)
{
    ...
    //os_printf("MGC_DrcServiceDefaultEnd: bVal = 0x%x\r\n", bVal);
    ...
    else
    {
		...
		{
                      os_printf("MGC_DrcServiceDefaultEnd: MGC_M_CSR0_RXPKTRDY\r\n");
		        pPort->bEnd0Status = FALSE;
		        /* receive */
                    os_printf("MGC_DrcServiceDefaultEnd: pPort->pFunctionClient = 0x%p, "
                   ...
                   "pPort->wSetupDataIndex = 0x%x\r\n",
					...
					wSpace = pPort->pFunctionClient ?(pPort->pFunctionClient->wControlBufferLength - pPort->wSetupDataIndex) : 0;
                    os_printf("MGC_DrcServiceDefaultEnd: wCount = 0x%x, wSpace = 0x%x\r\n", wCount, wSpace);
                    //UART_PRINTF("wCount=0x%x, wSpace=0x%x\r\n", wCount, wSpace);
		    wFifoCount = MUSB_MIN(wCount, wSpace);
		    /* stall on potential overflow or get data */
		    if (!pPort->pFunctionClient || (wCount > wFifoCount))
		    {
                        /* stall */
                        os_printf("MGC_DrcServiceDefaultEnd: send stall\r\n");
                        //UART_PRINTF("send stall\r\n");
		    ...
		    else
		    {
                ...
                os_printf("MGC_DrcServiceDefaultEnd: pFifoDest = 0x%p, "
                ...
                #endif
                os_printf("MGC_DrcServiceDefaultEnd: pPort->pfUnloadFifo = 0x%p\r\n", pPort->pfUnloadFifo);
                if (pPort->pfUnloadFifo)
                {
                    pPort->pfUnloadFifo(pPort, 0, wFifoCount, pFifoDest);
                }
                pPort->wSetupDataIndex += wFifoCount;
                os_printf("MGC_DrcServiceDefaultEnd: pPort->wSetupDataIndex = 0x%x\r\n", pPort->wSetupDataIndex);
                /* check request */
                pRequest = (MUSB_DeviceRequest*)pPort->pFunctionClient->pControlBuffer;
                
                if (pRequest)
                {
                    ///os_printf("MGC_DrcServiceDefaultEnd: pRequest->bmRequestType = 0x%x\r\n", pRequest->bmRequestType);
                    ///os_printf("MGC_DrcServiceDefaultEnd: pRequest->wLength = 0x%x\r\n", pRequest->wLength);
                    ...
                    if ((pRequest->bmRequestType & MUSB_DIR_IN) ||
                                ((pPort->wSetupDataIndex - 8) >= MUSB_SWAP16P((uint8_t*)&(pRequest->wLength))))
                            {
                                ...
                                if (pPort->bParse)
                                {
                                        ...
                                        if (!MGC_FunctionParseSetup(pPort, &status))  //190401
                                        {
                                            /* callback */
...
static void MGC_DrcFlushAll(MGC_Port* pPort)
{
    ...
    os_printf("MGC_DrcFlushAll: pPort = %p\r\n", pPort);
...
static void MGC_HostResetStart(void* pParam, uint16_t wTimer)
{
	...
	os_printf("MGC_HostResetStart: MGC_HostResetStart = %p\r\n", MGC_HostResetStart);
...
//	os_printf("toOtg=%x\r\n",toOtg);
	...
	*/
	...
	{
		os_printf("MGC_DrcChangeOtgState: MUSB_A_HOST: start timer\r\n");
	//MGC_NoneArmTimer
...
uint8_t MGC_DrcUsbIsr(MGC_Port* pPort, uint8_t bIntrUsbVal)
{
  ...
  //  uart_printf("MGC_DrcUsbIsr: pPort = 0x%p, bIntrUsbVal = 0x%x\r\n", pPort, bIntrUsbVal);
    if (bIntrUsbVal)
    {
		...
		if (bIntrUsbVal & MGC_M_INTR_DISCONNECT)
		{
		    ...
		    if (!pPort->bWantReset)
		    {
		    	//msg_put((uint32)MSG_USB_DETATCH);
...
memory_usage_show();
		    ...
		    /*device mode*/
                ...
                os_printf("MGC_DrcUsbIsr: pPort->bOtgState = 0x%x\r\n", pPort->bOtgState);
				switch(pPort->bOtgState)
				{
					case MUSB_AB_IDLE:
				    ...
				    pEnd->dwWaitFrameCount)
				{
				    ...
				    if (0 == --pEnd->dwWaitFrameCount)
				    {
						pPort->pfProgramStartTransmit(pPort, pEnd,
						    pIsochIrp->pBuffer, pIsochIrp->adwLength, pIsochIrp);
				    }
				}
		    }
...
uint32_t MGC_DrcBsr(void* pParam)
{
    ...
    os_printf("MGC_DrcBsr: bOk = %d\r\n", bOk);
    while (bOk)
    {
    	os_printf("MGC_DrcBsr: item.bCause = 0x%x\r\n", item.bCause);
		switch(item.bCause)
		{
...
//插入U盘后只是把此事件的消息传递给上层函数，何时开始枚举由上层决定
        ...
        os_printf("MGC_DrcBsr: MGC_BSR_CAUSE_DISCONNECT: pPort->bOtgState=0x%x\r\n", pPort->bOtgState);
#if 0
...
static void MGC_OtgStateGetId(MGC_Port* pPort, uint8_t bIsReset)
{
    os_printf("MGC_OtgStateGetId: pPort->bIsHost=%x, pPort->bconnectorID=%x, bIsreset=%x\r\n",
        pPort->bIsHost, pPort->bConnectorId, bIsReset);
    pPort->bWantSuspend = FALSE;
    pPort->bWantSession = TRUE;
    os_printf("MGC_OtgStateGetId: pPort->pfProgramBusState = 0x%p\r\n", pPort->pfProgramBusState);
    ...
    */
    os_printf("pPort->pfReadBusState = 0x%p\r\n", pPort->pfReadBusState);
				      ...
				      MUSB_OtgClient* pOtgClient)
{
    ...
    if (!pImplPort->bSessionActive)
    {
        ...
        if (pFunctionClient)
        {
            os_printf("MGC_aDescriptorData = 0x%p, ulMGC_aDescriptorDataLen = 0x%x\r\n",
                       pFunctionClient->pStandardDescriptors, pFunctionClient->wDescriptorBufferLength);
            os_printf("MGC_aHighSpeedDescriptorData = 0x%p, ulMGC_aHighSpeedDescriptorDataLen = 0x%x\r\n",
                       pFunctionClient->pHighSpeedDescriptors, pFunctionClient->wHighSpeedDescriptorLength);
            os_printf("ulgHidKeyboardReportDescriptorLen = 0x%x\r\n", ulgHidKeyboardReportDescriptorLen);
            //os_printf("ulgHidMouseReportDescriptorLen = 0x%x\r\n", ulgHidMouseReportDescriptorLen);
            //os_printf("ulgHidRMCVoiceReportDescriptorLen = 0x%x\r\n", ulgHidRMCVoiceReportDescriptorLen);
            /* function client requires verification */
            dwStatus = MGC_FunctionRegisterClient(pImplPort, pFunctionClient);
            os_printf("MUSB_RegisterOtgClient: dwStatus = 0x%lx\r\n", dwStatus);
        }
        ...
        if (0 == dwStatus)
        {
            ...
            if (pFunctionClient)
            {
                pOtgDesc = (MUSB_OtgDescriptor*)MGC_FindDescriptor(pFunctionClient->pStandardDescriptors,
...
void MUSB_RelinquishHost(MUSB_BusHandle hBus)
{
	...
	/* suspend bus */
    	...
    	pServices->pfArmTimer(pServices->pPrivateData, 0, 2, FALSE,MGC_RemoveSuspend);
    }
}
...
//	MGC_FdrcReadBusState()
	...
	{
		os_printf("MUSB_Start_Fun: =====111111===\r\n");
		MGC_DrcChangeOtgState(MUSB_A_HOST, pPort);
		os_printf("MUSB_Start_Fun: =====222222===\r\n");
	}
}
