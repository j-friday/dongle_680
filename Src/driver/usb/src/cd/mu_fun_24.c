...
unsigned char ucAudioRecordOnOff = 0;
			     ...
			     const MUSB_EndpointDescriptor* pEndpoint)
{
    ...
    os_printf("MGC_FunctionPreparePipe: pEndpoint->bmAttributes = 0x%x\r\n", pEndpoint->bmAttributes);
    pPipe->hSession = hBus;
    pPipe->pPort = pPort;
				       ...
				       uint8_t bBind)
{
    ...
    os_printf("MGC_FunctionBindConfig\r\n");
    ...
    for (bIfaceIndex = 0; !dwStatus; bIfaceIndex++)
    {
        ...
        os_printf("MGC_FunctionBindConfig: pIfaceDesc = 0x%p, pConfigDesc= 0x%p\r\n", pIfaceDesc, pConfigDesc);
        os_printf("MGC_FunctionBindConfig: wLength = 0x%x, wLimit = 0x%x\r\n", wLength, wLimit);
        os_printf("MGC_FunctionBindConfig: pIfaceDesc->bNumEndpoints = %d\r\n", pIfaceDesc->bNumEndpoints);
            ...
            bEndIndex++)
        {
            ...
            MUSB_MemCopy((void*)&(Endpoint.UsbDescriptor),
                         (void*)pEndDesc, sizeof(MUSB_EndpointDescriptor));
            os_printf("MGC_FunctionBindConfig: bReallyBind = 0x%x\r\n", bReallyBind);
            // MGC_FdrcBindEndpoint()
            pResource = pPort->pfBindEndpoint(pPort, &Endpoint, NULL, bReallyBind);
//            os_printf("MGC_FunctionBindConfig: pResource = 0x%p, bReallyBind = 0x%x\r\n", pResource, bReallyBind);
            /* break on failure */
            if (!pResource)
            {
                dwStatus = MUSB_STATUS_NO_RESOURCES;
					    ...
					    uint8_t bVerify)
{
    ...
    os_printf("MGC_FunctionVerifyResources: ----1\r\n");
    os_printf("MGC_FunctionVerifyResources: pDescriptorBuffer = 0x%p, wDescriptorBufferLength = 0x%x, "
    ...
    for (bConfigIndex = 0;
         ...
         bConfigIndex ++)
    {
        os_printf("MGC_FunctionVerifyResources: ----2\r\n");
		pDesc = MGC_FindDescriptor(pDescriptorBuffer, wDescriptorBufferLength,
		    bType, bConfigIndex);
        os_printf("MGC_FunctionVerifyResources: pDesc = 0x%p\r\n", pDesc);
		if (pDesc)
		{
		    /* config found */
		    ...
		    if (bVerify)
		    {
                os_printf("MGC_FunctionVerifyResources: ----3\r\n");
				dwStatus = MGC_FunctionBindConfig(pPort, pConfigDesc, FALSE);
                os_printf("MGC_FunctionVerifyResources: dwStatus = 0x%lx\r\n", dwStatus);
		    }
		    else
		    {
...
//	static MGC_Pipe pipe[3];
	...
	os_printf("bnumconfiguration = 0x%x, sizeof(MUSB_ConfigurationDescriptor) = 0x%x\r\n",
   ...
   if (pPort->apConfigDescriptors)
    {
		...
		dwStatus = MGC_FunctionVerifyResources(pPort, pClient->pStandardDescriptors,
                        ...
                        MUSB_DT_CONFIG, TRUE);
		os_printf("MGC_FunctionRegisterClient 1 dwstatus :0x%x\r\n",dwStatus);
		if (!dwStatus && (pClient->wHighSpeedDescriptorLength > sizeof(MUSB_QualifierDescriptor)))
		{
            ...
            os_printf("MGC_FunctionRegisterClient 2 dwstatus :0x%x\r\n",dwStatus);
		}
    }
    else
    {
        UART_PRINTF("MGC_FunctionRegisterClient: MUSB_MemAlloc 1 failed\r\n");
        dwStatus = MUSB_STATUS_NO_MEMORY;
    }

    /* if we are OK; allocate pipe memory now */
    os_printf("MGC_FunctionRegisterClient: pPort->bMaxFunctionEnds = %d\r\n", pPort->bMaxFunctionEnds);
    if (pPort->bMaxFunctionEnds && (0 == dwStatus))
    {
//   	pPort->apPipe = (MGC_Pipe**)MGC_Pipe_pointer;

        os_printf("MGC_FunctionRegisterClient: pPort->bMaxFunctionEnds = 0x%x, sizeof(MGC_Pipe*)=0x%x\r\n",
	...
	//	    pPipe = pipe;
			...
			{
				UART_PRINTF("MGC_FunctionRegisterClient: MUSB_MemAlloc 3 failed\r\n");
		...
		{
            	UART_PRINTF("MGC_FunctionRegisterClient: MUSB_MemAlloc 2 failed\r\n");
				dwStatus = MUSB_STATUS_NO_MEMORY;
		}
    }
...
//    os_printf("\r\n");
//    os_printf("dwStatus=0x%x\r\n", dwStatus);
//    os_printf("apConfigDesc[0]=0x%p\r\n", pPort->apConfigDescriptors[0]);
...
void MGC_FunctionSpeedSet(MGC_Port* pPort)
{
    ...
    os_printf("MGC_FunctionSpeedSet: pPort = 0x%p, pPort->pFunctionClient = 0x%p\r\n", pPort, pPort->pFunctionClient);
    if (pPort->pFunctionClient)
    {
        ...
        os_printf("MGC_FunctionSpeedSet: pDeviceDesc = 0x%p, pQualifierDesc = 0x%p, pPort->bIsHighSpeed = %d\r\n",
...
extern const uint8_t gHidReportDescriptor2[];
extern unsigned long ulgHidReportDescriptor2Len;
...
extern const uint8_t gHidISOCVoiceReportDescriptor[];
extern unsigned long ulgHidISOCVoiceReportDescriptorLen;
extern const uint8_t gIsocCurrentDescriptor[];
extern unsigned long ulgIsocCurrentDescriptorLen;
...
uint8_t MGC_FunctionParseSetup(MGC_Port* pPort, uint8_t* pbStatus)
{
    ...
    *pbStatus = MUSB_STATUS_OK;
    ...
    if (pRequest && (MUSB_TYPE_STANDARD == (pRequest->bmRequestType & MUSB_TYPE_MASK)))
    {
        ...
        os_printf("MGC_FunctionParseSetup: pRequest->bmRequestType = 0x%x, bRecip = 0x%x, wValue = 0x%x, wIndex = 0x%x\r\n",
                pRequest->bmRequestType, bRecip, wValue, wIndex);
        switch (pRequest->bRequest)
        {
        case MUSB_REQ_GET_STATUS:
            os_printf("MGC_FunctionParseSetup: MUSB_REQ_GET_STATUS\r\n");
        ...
        case MUSB_REQ_CLEAR_FEATURE:
            os_printf("MGC_FunctionParseSetup: MUSB_REQ_CLEAR_FEATURE\r\n");
        ...
        case MUSB_REQ_SET_FEATURE:
            os_printf("MGC_FunctionParseSetup: MUSB_REQ_SET_FEATURE\r\n");
            /* we handle device, interface (nothing defined) and endpoint */
            if ((pPort->bUsbState >= MUSB_ADDRESS) || (0 == wIndex))
            {
                switch (bRecip)
    ...
    //        GPIO_Output(26, FALSE);
            os_printf("MGC_FunctionParseSetup: MUSB_REQ_SET_ADDRESS\r\n");
            bOurs = TRUE;
            pPort->wSetupDataSize = 0;
            os_printf("MGC_FunctionParseSetup: pPort->bUsbState = 0x%x, pPort->bSetAddress = 0x%x\r\n",
                    pPort->bUsbState, pPort->bSetAddress);
            if ((pPort->bUsbState < MUSB_ADDRESS) || pPort->bSetAddress)
            {
                os_printf("MGC_FunctionParseSetup: MGC_FunctionChangeState\r\n");
            ...
            {
                os_printf("MGC_FunctionParseSetup: MUSB_STATUS_STALLED!!!!!!\r\n");
                *pbStatus = MUSB_STATUS_STALLED;
            }
            break;

        case MUSB_REQ_GET_DESCRIPTOR:   ///190328
            os_printf("MGC_FunctionParseSetup: MUSB_REQ_GET_DESCRIPTOR\r\n");
            os_printf("GET_DESCRIPTOR:bRecip=%d, wValue=0x%x\r\n", bRecip, wValue);
            if (MUSB_RECIP_DEVICE == bRecip)
            {
                bOurs = TRUE;
                ...
                {
                    os_printf("dance start bType=0x%x\r\n", bType);
                    switch (bType)
                    {
                    case MUSB_DT_DEVICE:
                ...
                {
                    os_printf("bOK=%d, pScan[1]=0x%x, bQueryType=0x%x\r\n", bOK, pScan[1], bQueryType);
                ...
                if (pScan < pScanLimit)
                {
                    os_printf("dance end bType=0x%x, bSwitchDescriptor=%d\r\n", bType, bSwitchDescriptor);
                    ...
                    if (bSwitchDescriptor)
                    {
                        ...
                        switch (bType)
                        {
                            ...
                            {
                                int i;
                                os_printf("pPort->wSetupDataSize=0x%x\r\n", pPort->wSetupDataSize);
                                os_printf("pPort->pSetupData:\r\n");
                                for (i=0; i<pPort->wSetupDataSize; i++)
                                    os_printf("%02x ", pPort->pSetupData[i]);
                                os_printf("\r\n");
                            }
                    ...
                    else
                    {
                        ...
                        os_printf("no dance: pPort->wSetupDataSize=0x%x\r\n", pPort->wSetupDataSize);
                        if (0)
                        {
							int i;
                            os_printf("no dance: pPort->wSetupDataSize=0x%x\r\n", pPort->wSetupDataSize);
                            os_printf("no dance: pPort->pSetupData:\r\n");
                            for (i=0; i<pPort->wSetupDataSize; i++)
                                os_printf("%02x ", pPort->pSetupData[i]);
                            os_printf("\r\n");
                        }
...
//                        pPort->wSetupDataSize = ulgHidReportDescriptorLen;
                        ...
                        switch (wIndex)
                        {
//                        	pPort->wSetupDataSize = ulgHidISOCVoiceReportDescriptorLen;
//                        	break;
        ...
        case MUSB_REQ_GET_CONFIGURATION:
            os_printf("MGC_FunctionParseSetup: MUSB_REQ_GET_CONFIGURATION\r\n");
            bOurs = TRUE;
        ...
        case MUSB_REQ_SET_CONFIGURATION:
            os_printf("MGC_FunctionParseSetup: MUSB_REQ_SET_CONFIGURATION\r\n");
//            os_printf("SET_CONFIG:bUsbState=%d, wValue=%d, bIsHS=%d\r\n", pPort->bUsbState, wValue, pPort->bIsHighSpeed);
            bOurs = TRUE;
            pPort->wSetupDataSize = 0;
            bOK = FALSE;
...
//            os_printf("bOK=%d\r\n", bOK);
        ...
        case MUSB_REQ_GET_INTERFACE:
            os_printf("MGC_FunctionParseSetup: MUSB_REQ_GET_INTERFACE\r\n");
        case MUSB_REQ_SET_INTERFACE:
            os_printf("MGC_FunctionParseSetup: MUSB_REQ_SET_INTERFACE\r\n");
            pPort->wSetupDataSize = 0;
            ...
            /* re-bind */
                ...
                else		// by gwf    此为录音机程序定制，不可通用
                {
                	if (wIndex == 1)
                	{
                		if (wValue == 1)
                		{
                			ucAudioRecordOnOff = 1;
                		}
                		else
                		{
                			ucAudioRecordOnOff = 0;
                		}
                	}
                }
            }
    ...
    {
//        os_printf("CLASS bRequest=%x\r\n", pRequest->bRequest);
//        os_printf("CLASS\r\n");
        bOurs = TRUE;
        wValue = ((((pRequest->wValue) & 0x00FF) << 8) | (((pRequest->wValue) & 0xFF00) >> 8));
        wIndex = ((((pRequest->wIndex) & 0x00FF) << 8) | (((pRequest->wIndex) & 0xFF00) >> 8));
        os_printf("CLASS: pRequest->bRequest = 0x%x, wIndex = 0x%x, wValue = 0x%x,length=%x\r\n",
                pRequest->bRequest, wIndex, wValue,pRequest->wLength);
        switch (pRequest->bRequest)
        {
...
//            pPort->wSetupDataSize = ulgHidReportDescriptorLen;
        ...
        case MUSB_CLASS_SET_REPORT:
            // TODO
            //USBD_CMD_Process(&pPort->pSetupData[8]);
            pPort->wSetupDataSize = 0;
            break;
...
//            MUSB_MemCopy(pPort->pSetupData, gIsocCurrentDescriptor, ulgIsocCurrentDescriptorLen);
//            pPort->wSetupDataSize = ulgIsocCurrentDescriptorLen;
            break;
...
void MGC_FunctionChangeState(MGC_Port* pPort, MUSB_State State)
{
    ...
    if (pPort->bUsbState != State)
    {
		...
		if (pPort->pFunctionClient)//就是 MGC_MsdFunctionClient
		{
