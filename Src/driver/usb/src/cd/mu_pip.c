				  ...
				  uint8_t bDescriptorType, uint8_t bIndex)
{
    ...
    os_printf("MGC_FindDescriptor: pHeader->bDescriptorType = 0x%x, bDescriptorType = 0x%x, "
    ...
    {
        os_printf("MGC_FindDescriptor: wOffset = 0x%x, wFoundIndex = %d\r\n", wOffset, wFoundIndex);
		pHeader = (MUSB_DescriptorHeader*)((uint8_t*)pBuffer + wOffset);
        os_printf("MGC_FindDescriptor: pHeader = 0x%p, pHeader->bLength = 0x%x\r\n", pHeader, pHeader->bLength);
			       ...
			       uint8_t bContinue, void* pCurrentIrp)
{
    ...
    //os_printf("MGC_PipePacketReceived\r\n");
    ...
    switch(pEnd->bRxTrafficType & MUSB_ENDPOINT_XFERTYPE_MASK)
    {
		...
		pIsochIrp->adwActualLength = wPacketSize;
		pIsochIrp->adwStatus = 0;
        if (0 != wPacketSize)
        {
            pIsochIrp->wCurrentFrame++;
        }
		dwReqLength = pIsochIrp->adwLength;
    ...
    case MUSB_ENDPOINT_XFER_ISOC:
    	...
    	{
    		pIsochIrp->adwActualLength -= ISOC_PACK_HEAD_LEN;
...
uint8_t MGC_PipeTransmitReady(MGC_Port* pPort, MGC_EndpointResource* pEnd)
{
    ...
    if (pEnd->pTxIrp)
    {
		...
		case MUSB_ENDPOINT_XFER_ISOC:
		    ...
		    pGenIrp = pIsochIrp;
		    //wFrameIndex = pIsochIrp->wCurrentFrame;
		    pIsochIrp->adwActualLength = pIsochIrp->adwLength;
		    pIsochIrp->adwStatus = 0;
		    pIsochIrp->wCurrentFrame++;
		    dwReqLength = pIsochIrp->adwLength;
