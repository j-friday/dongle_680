			   ...
			   void* pStaticBuffer)
{
    ...
    {
		UART_PRINTF("MUSB_ArrayInit: malloc failed\r\n");
    }
    return pResult;
}
...
{
    void* pResult = NULL;
    os_printf("MUSB_ArrayFetch: pArray = 0x%p, wIndex = 0x%x\r\n", pArray, wIndex);
    if (pArray)
    {
//        os_printf("MUSB_ArrayFetch: pArray->wItemCount = 0x%x\r\n", pArray->wItemCount);
//        os_printf("MUSB_ArrayFetch: pArray->wStaticItemCount = 0x%x\r\n", pArray->wStaticItemCount);
...
uint8_t MUSB_ArrayAppend(MUSB_Array* pArray, const void* pItem)
{
    ...
    os_printf("MUSB_ArrayAppend: pArray = 0x%p, pItem = 0x%p, pArray->wItemCount = %d\r\n",
    ...
    else
    {
        ...
        pDest = pArray->pDynamicBuffer =  MUSB_MemAlloc(wNewCount * pArray->wItemSize);

      //  pDest = ke_malloc(wNewCount * pArray->wItemSize,3);
        if (pDest)
        {
            bResult = TRUE;
    ...
    if (bResult)
    {
        ...
        pArray->wItemCount++;
//        os_printf("MUSB_ArrayAppend: pArray->wItemCount = 0x%x\r\n", pArray->wItemCount);
    }
    return bResult;
}
				  ...
				  uint32_t dwCount)
{
    ...
    if (pNext == NULL)
    {
		UART_PRINTF("MGC_ListInsertItem: malloc failed\r\n");
    }
    if (pPos && pNext)
    {
	bOK = TRUE;
			    ...
			    unsigned long dwCount)
{
    ...
    os_printf("MUSB_ListAppendItem: pList = 0x%p, pList->pItem = 0x%p, pList->pNext = 0x%p, pList->dwCount = 0x%lx\r\n",
			    ...
			    void* pItem, unsigned long dwCount)
{
	...
	{
		UART_PRINTF("MUSB_ListInsertItem: malloc failed\r\n");
	}
    }
    else
    {
