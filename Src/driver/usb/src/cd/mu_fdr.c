...
uint32_t MGC_FdrcStart(MGC_Controller* pController)
{
    ...
    os_printf("MGC_FdrcStart: MGC_O_FDRC_INTRTX1E = 0x%x\r\n", MGC_Read8(pBase, MGC_O_FDRC_INTRTX1E));
    os_printf("MGC_FdrcStart: MGC_O_FDRC_INTRRX1E = 0x%x\r\n", MGC_Read8(pBase, MGC_O_FDRC_INTRRX1E));
    os_printf("MGC_FdrcStart: MGC_O_FDRC_INTRUSBE = 0x%x\r\n", MGC_Read8(pBase, MGC_O_FDRC_INTRUSBE));
...
uint32_t MGC_FdrcReadBusState(MGC_Port* pPort)
{
	...
	os_printf("MGC_FdrcReadBusState: bIsHost=%x\r\n",pPort->bIsHost);
...
uint32_t MGC_FdrcProgramBusState(MGC_Port* pPort)
{
    ...
    os_printf("MGC_FdrcProgramBusState: MGC_FdrcProgramBusState = 0x%p, pPort->bWantReset=%d, "
					   ...
					   uint8_t bBind)
{
...
#endif
    os_printf("MGC_FdrcBindEndpoint: pEnd = 0x%p, bBind = %d\r\n", pEnd, bBind);
    if (pEnd && bBind)
    {
...
#ifdef MUSB_ISO
        //os_printf("MGC_FdrcBindEndpoint: pPort->bIsHost = %d, bTrafficType = 0x%x\r\n", pPort->bIsHost, pEnd->bTrafficType);
		if (!pPort->bIsHost && ((pEnd->bTrafficType & MUSB_ENDPOINT_XFERTYPE_MASK) == MUSB_ENDPOINT_XFER_ISOC))
		{
	...
	else
	{
		...
		else
		{
		   ...
		   // csr2 |= MGC_M_RXCSR2_AUTOCLEAR; ///190404
#endif
		}
		MGC_Write8(pBase, MGC_O_FDRC_TXCSR2, MGC_Read8(pBase, MGC_O_FDRC_TXCSR2) & (~MGC_M_TXCSR2_MODE));/* mode bit=0 for RX */
        //os_printf("MGC_FdrcBindEndpoint: MGC_O_FDRC_TXCSR2 = 0x%x\r\n", MGC_Read8(pBase, MGC_O_FDRC_TXCSR2));
		MGC_Write8(pBase, MGC_O_FDRC_RXCSR2, csr2);
        //os_printf("MGC_FdrcBindEndpoint: MGC_O_FDRC_RXCSR2 = 0x%x\r\n", csr2);
...
uint8_t MGC_FdrcServiceDefaultEnd(MGC_Port* pPort, MGC_BsrItem* pItem)
{
	...
	//os_printf("MGC_FdrcServiceDefaultEnd: endpoint 0 isr handle: wCsrVal=0x%x, wcount=0x%x, pPort->bIsHost=0x%x\r\n",
...
//	os_printf("wcsrval = %x,wcount=%x\r\n",wCsrVal,wCount);
    ...
    else if (!wCount && pPort->bEnd0Status)
    {
        ...
        os_printf("MGC_FdrcServiceDefaultEnd: pPort->bSetAddress = 0x%x\r\n", pPort->bSetAddress);
    ...
    os_printf("MGC_FdrcServiceDefaultEnd: bError = 0x%x\r\n", bError);
    if (bError)
    {
		if (pPort->bIsHost)
		{
