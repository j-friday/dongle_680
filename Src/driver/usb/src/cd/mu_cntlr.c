...
//	    pWrapper = NULL;
 ...
 //   if (pWrapper)
    ...
    os_printf("MUSB_NewController: wType 1 = %d\r\n", wType);
	if (!wType)
	{
	    wType = MGC_DiscoverController(pControllerAddressBsr);
	}
    os_printf("MUSB_NewController: wType 2 = %d\r\n", wType);
    ...
    os_printf("MUSB_NewController: pWrapper->PortImpl.bEndCount = %d\r\n", pWrapper->PortImpl.bEndCount);
	for (bEnd = 0; bEnd < pWrapper->PortImpl.bEndCount; bEnd++)
	{
	    Stats.dwOverheadMax = 0L;
	    Stats.dwOverheadMin = 0xffffffffL;
        os_printf("MUSB_NewController: bEnd = %d\r\n", bEnd);
	    MUSB_ArrayAppend(&(pWrapper->PortImpl.OverheadStats), &Stats);
	}
    os_printf("MUSB_NewController: MUSB_ArrayAppend End\r\n");
#endif
	/* ready */
			      ...
			      )
{
    ...
    os_printf("MUSB_StartController\r\n");
    if (pController)
    {
        pImpl = (MGC_Controller*)pController->pPrivateData;
    }
