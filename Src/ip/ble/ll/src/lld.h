...
struct lld_adv_params
{
    ...
    /// RAL enabled state (True: resolvable addresses enabled | False: resolvable addresses disabled)
    bool ral_en;
...
struct lld_per_adv_cte_params
{
    ...
    /// Antenna IDs
    uint8_t     antenna_id[BLE_ANTENNA_NB];
};
...
struct lld_scan_params
{
    ...
    /// Scan period (N * 1.28 sec) | 0x0000: Periodic scanning disabled
    uint16_t period;
};
 ...
 * @brief Get local SCA
 *
 * @return     local_sca     Local SCA (@see enum SCA)
 ****************************************************************************************
 */
...
uint8_t lld_con_evt_time_get(uint8_t link_id, uint16_t* con_evt_cnt, uint32_t* con_evt_time_hs, uint16_t* con_evt_time_hus);
 ...
 * @param[in] req_cte_type            Requested CTE type (0: AOA | 1: AOD-1us | 2: AOD-2us)
 *****************************************************************************************
 */
void lld_con_cte_rx_en(uint8_t link_id, uint8_t slot_dur, uint8_t switching_pattern_len, uint8_t req_cte_type);
