...
////Debug
extern volatile uint32_t ld_TmpAddr;
extern volatile uint16_t ld_TmpVal;
...
////Debug
ld_TmpAddr = EM_BT_FRCNTL_ADDR + elt_idx * REG_EM_BT_CS_SIZE;
ld_TmpVal  = ((uint16_t)fpoll << 15) | ((uint16_t)fwmspatt << 12) | ((uint16_t)txbsyen << 10) | ((uint16_t)rxbsyen << 9) | ((uint16_t)dnabort << 8) | ((uint16_t)samen << 7) | ((uint16_t)lsamdsb << 6) | ((uint16_t)format << 0);
////uart_printf("0x%X, 0x%X\r\n", ld_TmpAddr, ld_TmpVal);
}
...
////debug
ld_TmpAddr = EM_BT_BDADDR_ADDR + elt_idx * REG_EM_BT_CS_SIZE + reg_idx * 2;
ld_TmpVal = (uint16_t)bdaddr << 0;
////uart_printf("0x%X, 0x%X\r\n", ld_TmpAddr, ld_TmpVal);
}
...
////debug
ld_TmpAddr = EM_BT_BCH0_ADDR + elt_idx * REG_EM_BT_CS_SIZE;
ld_TmpVal = (uint16_t)bch0 << 0;
////uart_printf("0x%X, 0x%X\r\n", ld_TmpAddr, ld_TmpVal);
}
