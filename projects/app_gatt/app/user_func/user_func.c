...
uint8_t hexvalue[145]= 
{
    ...
    0x75,0x76,0x77,0x78,0x79,0x7A,
    0x2D,0x3D,0x5B,0x5D,0x5C,0x3B,0x27,0x2C,0x2E,0x2F,0x60,
    13,7,15,8,19,9,11,32,2,22,23,24,25,26,
...
uint8_t keyvalue[98] = 
{
    ...
    0x18,0x19,0x1A,0x1B,0x1C,0x1D,
    0x2D,0x2E,0x2F,0x30,0x31,0x33,0x34,0x36,0x37,0x38,0x35,
	0x28,0x28,0x29,0x2A,0x2A,0x2B,0x2B,0x2C,0x39,0x3A,0x3B,0x3C,0x3D,0x3E,
	0x3F,0x40,0x41,0x42,0x43,0x44,0x45,0x49,
	0x4A,0x4B,0x4C,0x4D,0x4E,0x4F,0x50,0x51,0x52,0x53,0x54,0x55,
	0x56,0x57,0x58,0xe6,0xe4,0x46,0x58,0xe2,0xe6,0xe0,0xe4,0xe1,0xe5,
	0x2b,0xab,0x28,0x29
};
#endif
//0-9,a-z,-=[]\;',./ keyvalue
    ...
    if(ble_rx_data_len > 0)
    {
        if(cable_sel_v == 0)
        {
            if((usb_send_totail < ble_rx_data_len) && (check_b_isTRxing() == FALSE))//for(uint16_t i = 0; i< ble_rx_data_len; i++)
            {
                ...
                memset(keybuf,0x0,8);        
                for(uint8_t j = 0; j< 145; j++)
                {
                    if((ble_data_buf[usb_send_totail] == hexvalue[j]) && (j < 98))/*正常按键*/
                    {
                        keybuf[2] = keyvalue[j];
                    ...
                    else if((ble_data_buf[usb_send_totail] == hexvalue[j]) && (j >= 98))/*shift组合按键*/
                    {
                        //keybuf[0] = 0x02;
                        //notify_data.notify_standard_key_status=1;
