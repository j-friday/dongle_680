...
__STATIC void cli_main(void)
{
    ...
    {
        if((NULL != strstr((const char *)AT_RX_BUFFER + AT_RX_INDEX - 13,"AT+ATMODE=0"))
            || (NULL != strstr((const char *)AT_RX_BUFFER + AT_RX_INDEX - 13,"AT+ATMODE=1"))
            || (NULL != strstr((const char *)AT_RX_BUFFER + AT_RX_INDEX - 13,"AT+ATMODE=2")))
        {                        
            set_at_mode(AT_RX_BUFFER[10]-0x30);
            if((AT_RX_BUFFER[10]-0x30) == 1)cliexit = 0;
            else cliexit = 1;
            
            aos_cli_printf("\r\nOK\r\n");
            
        }
        else if(get_at_mode() == 2)
        {
            if(NULL == strstr((const char *)AT_RX_BUFFER + AT_RX_INDEX - 9,"AT+HELP"))
            {
                app_send_at_cmd(0, AT_RX_INDEX, AT_RX_BUFFER);
            }
        }
    }
#endif			
...
int aos_cli_register_commands(const struct cli_command *cmds, int num_cmds)
{
    ...
    for (i = 0; i < num_cmds; i++) {
        if ((err = aos_cli_register_command(cmds++)) != 0) {
            aos_cli_printf("register_commands err cmd nums:%d\r\n",cmds);
            return err;
        }
    }
...
int aos_cli_init(void)
{
    int ret;
   // cli = NULL;
//    aos_cli_printf("aos_cli_init ok:0x%x,0x%x \r\n",cli,&cli);
//    ret = ||Load$$RAM_DATA$$Base||;
//    aos_cli_printf("RAM_DATA base:0x%x,0x%x \r\n",Load$$RAM_DATA$$Base);
//    aos_cli_printf("RAM_DATA base:0x%x,0x%x \r\n",Image$$RAM_DATA$$Base);
//    aos_cli_printf("RAM_DATA base:0x%x,0x%x \r\n",Image$$RAM_DATA$$Length);
    ...
    {
        //aos_cli_printf("aos_cli_init ok:0x%x \r\n",cli);
...
{
    if (msg[0] != 0) {
        UART_SEND_AT((unsigned char *)msg,strlen(msg));
    }
...
__attribute__((weak)) int csp_printf(const char *fmt,...)
{
    ...
    if(get_at_rsp_ch())
    {
        app_send_at_cmd(0, ret, (uint8_t *)uart_buff);//
    }
    else
    {
        UART_SEND_AT(uart_buff, ret);
    }
