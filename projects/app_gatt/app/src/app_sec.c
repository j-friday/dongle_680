...
void app_sec_send_security_req(uint8_t conidx)
{
    ...
    #else
    cmd->auth      = GAP_AUTH_REQ_MITM_BOND;//GAP_AUTH_REQ_NO_MITM_BOND;//GAP_AUTH_REQ_NO_MITM_NO_BOND;
...
void app_sec_send_bond_cmd(uint8_t conidx)
{
    ...
    cmd->pairing.auth      = GAP_AUTH_REQ_MITM_BOND;//GAP_AUTH_REQ_NO_MITM_BOND;//
    cmd->pairing.iocap     = GAP_IO_CAP_KB_ONLY;//GAP_IO_CAP_DISPLAY_ONLY;//

    //cmd->pairing.auth      = GAP_AUTH_REQ_MITM_BOND;//GAP_AUTH_REQ_MITM_BOND;//GAP_AUTH_REQ_NO_MITM_BOND;//GAP_AUTH_REQ_MITM_NO_BOND;//;
    //cmd->pairing.iocap     = GAP_IO_CAP_KB_DISPLAY;//GAP_IO_CAP_NO_INPUT_NO_OUTPUT;//GAP_IO_CAP_DISPLAY_ONLY;//GAP_IO_CAP_NO_INPUT_NO_OUTPUT;
    cmd->pairing.key_size  = 16;
    cmd->pairing.oob       = GAP_OOB_AUTH_DATA_NOT_PRESENT;
    cmd->pairing.sec_req   = GAP_NO_SEC;//GAP_SEC2_NOAUTH_DATA_SGN;//GAP_SEC1_AUTH_PAIR_ENC;//GAP_SEC1_NOAUTH_PAIR_ENC;//GAP_NO_SEC;//
    cmd->pairing.ikey_dist = GAP_KDIST_ENCKEY | GAP_KDIST_IDKEY;// | GAP_KDIST_SIGNKEY | GAP_KDIST_LINKKEY;//|GAP_KDIST_LINKKEY|GAP_KDIST_SIGNKEY | GAP_KDIST_SIGNKEY
    cmd->pairing.rkey_dist = GAP_KDIST_ENCKEY | GAP_KDIST_IDKEY;// | GAP_KDIST_SIGNKEY | GAP_KDIST_LINKKEY | GAP_KDIST_SIGNKEY;//GAP_KDIST_ENCKEY | 
    ...
    switch (param->request)
    {
        case (GAPC_PAIRING_REQ):
        {
            ...
            #endif // !BLE_APP_AM0
            {
                ...
                cfm->data.pairing_feat.auth      = GAP_AUTH_REQ_MITM_BOND;//GAP_AUTH_REQ_NO_MITM_BOND;
                app_env.sec_con_enabled = false;
                ...
                #else
                cfm->data.pairing_feat.iocap     = GAP_IO_CAP_DISPLAY_ONLY;//GAP_IO_CAP_KB_ONLY;//GAP_IO_CAP_DISPLAY_ONLY;//GAP_IO_CAP_NO_INPUT_NO_OUTPUT;
                #endif //(BLE_APP_HT)
        ...
        case (GAPC_IRK_EXCH):
        {
            ...
            #if (NVDS_SUPPORT)
            if (rwip_param.get(PARAM_ID_BD_ADDRESS, &addr_len, cfm->data.irk.addr.addr.addr) == PARAM_OK)            
            #endif //(NVDS_SUPPORT)
            {
                ASSERT_ERR(0);
            }
        ...
        case (GAPC_TK_EXCH):
        {
            ...
            uint8_t pin_code_str[17];
            memset(pin_code_str, 0x00, 17);
            get_ble_pincode(pin_code_str);
            pin_code = atoi((const char *)pin_code_str);
    ...
    switch (param->info)
    {
        case (GAPC_PAIRING_SUCCEED):
        {
            ...
            if(app_env.role[conidx] == ROLE_MASTER)
            {
				...
				ke_timer_set(APP_ADAPTER_IN_HANDLE, TASK_APP, 100);
			#endif           
                                    ...
                                    ke_task_id_t const src_id)
{
    uint8_t conidx = KE_IDX_GET(src_id);
    // Drop the message
    bk_printf("\r\n%s msgid :%#x,auth:%#x", __func__, msgid,param->auth);
