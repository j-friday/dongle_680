...
enum KE_EVENT_TYPE
{
    ...
    KE_EVENT_ECC_MULTIPLICATION = 3,

    #if BT_EMB_PRESENT
    KE_EVENT_P192_PUB_KEY_GEN = 4,
    #endif // BT_EMB_PRESENT

    #if (BLE_MESH)
    KE_EVENT_BLE_MESH_DJOB = 5 ,
    #endif // (BLE_MESH)
    ...
    KE_EVENT_H4TL_TX = 10,
    #if (BLE_EMB_PRESENT || BT_EMB_PRESENT)
    KE_EVENT_H4TL_CMD_HDR_RX = 11,
    KE_EVENT_H4TL_CMD_PLD_RX = 12,
    #endif //(BLE_EMB_PRESENT || BT_EMB_PRESENT)
    #if (((BLE_EMB_PRESENT || BLE_HOST_PRESENT) && (BLE_CENTRAL || BLE_PERIPHERAL)) || BT_EMB_PRESENT)
    KE_EVENT_H4TL_ACL_HDR_RX = 13,
    #endif //(((BLE_EMB_PRESENT || BLE_HOST_PRESENT) && (BLE_CENTRAL || BLE_PERIPHERAL)) || BT_EMB_PRESENT)
    #endif //H4TL_SUPPORT

    #if (BLE_HOST_PRESENT)
    #if (BLE_L2CC)
    KE_EVENT_L2CAP_TX = 14,
    #endif //(BLE_L2CC)
    #endif// (BLE_HOST_PRESENT)

    #if BT_EMB_PRESENT
    KE_EVENT_BT_PSCAN_PROC = 15,
    #endif //BT_EMB_PRESENT

    #if (BLE_ISOOHCI)
    KE_EVENT_ISOOHCI_IN_DEFER = 16,
    KE_EVENT_ISOOHCI_OUT_DEFER = 17 ,
    #endif //(BLE_ISOOHCI)

  #if(BEKEN_TASK_ENABLE)
	KE_EVENT_BEKEN_TEST = 18,
  #endif
...
enum PARAM_ID
{
    ...
    /// Radio Jitter
    PARAM_ID_LPCLK_JITTER               = 0x04,
    /// External wake-up time
    PARAM_ID_EXT_WAKEUP_TIME            = 0x05,
    /// Oscillator wake-up time
    PARAM_ID_OSC_WAKEUP_TIME            = 0x06,
    /// Radio wake-up time
    PARAM_ID_RM_WAKEUP_TIME             = 0x07,
    /// UART baudrate
    PARAM_ID_UART_BAUDRATE              = 0x08,
    /// Enable sleep mode
    PARAM_ID_SLEEP_ENABLE               = 0x09,
    /// Enable External Wakeup
    PARAM_ID_EXT_WAKEUP_ENABLE          = 0x0A,
    /// SP Private Key 192
    PARAM_ID_SP_PRIVATE_KEY_P192        = 0x0B,
    /// SP Public Key 192
    PARAM_ID_SP_PUBLIC_KEY_P192         = 0x0C,

    /// Activity Move Configuration (enables/disables activity move for BLE connections and BT (e)SCO links)
    PARAM_ID_ACTIVITY_MOVE_CONFIG       = 0x0D,

    /// Enable/disable scanning for extended advertising PDUs
    PARAM_ID_SCAN_EXT_ADV               = 0x0E,

    /// Duration of the schedule reservation for long activities such as scan, inquiry, page, HDC advertising
    PARAM_ID_SCHED_SCAN_DUR             = 0x0F,

    /// Synchronous links configuration
    PARAM_ID_SYNC_CONFIG                = 0x10,
    /// PCM Settings
    PARAM_ID_PCM_SETTINGS               = 0x11,
    /// Sleep algorithm duration
    PARAM_ID_SLEEP_ALGO_DUR             = 0x12,
    /// Tracer configuration
    PARAM_ID_TRACER_CONFIG              = 0x13,

    /// RSSI threshold tags
    PARAM_ID_RSSI_HIGH_THR              = 0x14,
    PARAM_ID_RSSI_LOW_THR               = 0x15,
    PARAM_ID_RSSI_INTERF_THR            = 0x16,


    PARAM_ID_BT_LINK_KEY_FIRST          = 0x17,
    PARAM_ID_BT_LINK_KEY_LAST           = 0x26,

    PARAM_ID_BLE_LINK_KEY_FIRST         = 0x27,
    PARAM_ID_BLE_LINK_KEY_LAST          = 0x36,
    /// SC Private Key (Low Energy)
    PARAM_ID_LE_PRIVATE_KEY_P256        = 0x37,
    /// SC Public Key (Low Energy)
    PARAM_ID_LE_PUBLIC_KEY_P256         = 0x38,
    /// SC Debug: Used Fixed Private Key from NVDS (Low Energy)
    PARAM_ID_LE_DBG_FIXED_P256_KEY      = 0x39,
    /// SP Private Key (classic BT)
    PARAM_ID_SP_PRIVATE_KEY_P256        = 0x3A,
    /// SP Public Key (classic BT)
    PARAM_ID_SP_PUBLIC_KEY_P256         = 0x3B,

    /// LE Coded PHY 500 Kbps selection
    PARAM_ID_LE_CODED_PHY_500           = 0x3C,
